...
angular.module('titanApp')
    .controller('VideoEditorCtrl', function ($scope, $window, $http, $filter, $route, $rootScope, $location, video_idx, youtubeFactory, api_search_youtube, api_video, $interval, $timeout, Modal, api_item, $modalInstance, api_shape, api_titan_model, AuthService, api_make_titan_video, api_progress_process, api_item_position_detail, api_item_detail, ENV) {
        ...
        $scope.item_list_get = function(){
            var api_item_params = {};
            api_item_params['user_idx'] = AuthService.getIdx();
            api_item_params['fk_group_idx_sample'] = AuthService.getFkGroupIdx();
            ...
            api_shape.get(null, function (data) {
                if(data.status == 200){
                    ...
                    api_video.get(api_params, function (data) {
                        if(data.status == 200){
                            if(data.objects != undefined && data.objects != ''){
                                $scope.user_model = data.objects[0].user_model;
                                if($scope.user_model != 0){
                                    var um = $scope.item_model_type.filter(function (item_model) { return item_model.value == $scope.user_model });
                                    $scope.main_model = um[0];
                                    $scope.make_model_title = um[0].item;
                                    $rootScope.prev_model_item = "이전 사용한 A.I.모델";
                                }else{
                                    $scope.main_model = '';
                                    $scope.make_model_title = "선택 없음";
                                ...
                                var drop_video = confirm('사용자의 동영상이 존재하지 않습니다. 추가 하시겠습니까?(취소할 시, 공유된 상품 동영상을 사용합니다.)');
                                if(drop_video){
...
//        }
        ...
        $scope.video_click = function(item){
            if($scope.main_model != ''){
