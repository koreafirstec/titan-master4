...
IM_EXTENSIONS = ['png', 'jpg', 'bmp', 'jpeg']
...
def read_data(data_dir, image_size, pixels_per_grid=32, no_label=False, model_idx=False):
    """
    ...
    if no_label:
        ...
        for im_path in im_paths:
            ...
            anno_list = TB_ANNOTATION.query.filter_by(fk_model_idx=model_idx, image_name=name).all()
            anno = {}
            for an in anno_list:
                ...
                if class_name in anno:
                    anno[class_name].append(rect)
                else:
                    anno[class_name] = [rect]
